 ...
 * Cấu hình: window.MONA_QUOTE_CFG = { logoDark, logoWhite, site }
 */
(function () {
  'use strict';
  ...
  var MIN_LEN = 12;   // số ký tự tối thiểu của đoạn bôi đen
  var MAX_LEN = 600;  // quá dài thì cắt cho vừa card

  // Vùng cho phép bôi đen (nội dung bài/trang)
  var ALLOW = '.mona-content, .entry-content, .blog-large-content, article, main, [data-quote-source]';
  // Vùng loại trừ (không tạo quote ở nav/footer/form/nút...)
  // KHÔNG dùng [class*="form"] — match nhầm class body "single-format-standard"
  var DENY = 'header, footer, nav, aside, form, button, input, textarea, .popup, .menu-extra, .breadcrumb, .wpcf7, .contact-box';

  // Bảng nền: đa màu kiểu Brands VN + gradient brand MONA làm mặc định
  var THEMES = [
    { id: 'brand', label: 'MONA', grad: ['#7C0FD1', '#FF6E00'], fg: '#ffffff', logo: 'white' },
    { id: 'tim',   label: 'Tím',  bg: '#7C0FD1', fg: '#ffffff', logo: 'white' },
    { id: 'cam',   label: 'Cam',  bg: '#FF6E00', fg: '#ffffff', logo: 'white' },
    { id: 'den',   label: 'Đen',  bg: '#16121F', fg: '#ffffff', logo: 'white' },
  ...
  function buildModal() {
    ...
    THEMES.forEach(function (t) {
      ...
      b.dataset.id = t.id;
      b.title = t.label;
      if (t.grad) b.style.background = 'linear-gradient(135deg,' + t.grad[0] + ',' + t.grad[1] + ')';
  ...
  function fileName() {
    var slug = (state.title || 'mona-quote').toLowerCase()
      .normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/đ/g, 'd')
      .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 50);
    return 'mona-quote-' + (slug || 'share') + '.png';
  }
  function download() {
